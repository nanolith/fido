/**
 * \file test/config/test_fido_scanner_read_token.cpp
 *
 * \brief Test that we can read tokens from the scanner.
 *
 * \copyright 2026 Justin Handville.  Please see license.txt in this
 * distribution for the license terms under which this software is distributed.
 */

#include <fido/scanner.h>
#include <minunit/minunit.h>

TEST_SUITE(fido_scanner_read_token);

/**
 * \brief Reading from an empty string results in EOF.
 */
TEST(empty_string_eof)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = "";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read an open brace token.
 */
TEST(open_brace)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " { ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_OPEN_BRACE ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_OPEN_BRACE == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(1 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(2 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read a close brace token.
 */
TEST(close_brace)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " } ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_CLOSE_BRACE ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_CLOSE_BRACE == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(1 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(2 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read a role keyword token.
 */
TEST(role_keyword)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " role ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_KEYWORD_ROLE ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_KEYWORD_ROLE == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read a cmd keyword token.
 */
TEST(cmd_keyword)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " cmd ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_KEYWORD_CMD ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_KEYWORD_CMD == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read an as keyword token.
 */
TEST(as_keyword)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " as ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_KEYWORD_AS ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_KEYWORD_AS == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(2 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(3 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read an env keyword token.
 */
TEST(env_keyword)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " env ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_KEYWORD_ENV ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_KEYWORD_ENV == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(3 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(4 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read a permit keyword token.
 */
TEST(permit_keyword)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " permit ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_KEYWORD_PERMIT ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_KEYWORD_PERMIT == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read a deny keyword token.
 */
TEST(deny_keyword)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " deny ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_KEYWORD_DENY ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_KEYWORD_DENY == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read an add variable token.
 */
TEST(add_variable)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " +PATH ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_ADD_VARIABLE ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_ADD_VARIABLE == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(5 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(6 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read a username token.
 */
TEST(username)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " root ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_USERNAME ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_USERNAME == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(4 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(5 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read a group token.
 */
TEST(group)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = " :wheel ";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_GROUP ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_GROUP == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}

/**
 * \brief We can read a string token.
 */
TEST(string)
{
    fido_scanner* scanner = nullptr;
    fido_token_details details;
    const char* TEST_INPUT = R"( "test" )";

    /* Create the scanner instance. */
    TEST_ASSERT(0 == fido_scanner_create(&scanner, TEST_INPUT));

    /* read a token. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_STRING ==
            fido_scanner_read_token(&details, scanner));

    /* the details are correct. */
    TEST_EXPECT(FIDO_SCANNER_TOKEN_TYPE_STRING == details.type);
    TEST_EXPECT(1 == details.begin_index);
    TEST_EXPECT(6 == details.end_index);
    TEST_EXPECT(1 == details.begin_line);
    TEST_EXPECT(1 == details.end_line);
    TEST_EXPECT(2 == details.begin_col);
    TEST_EXPECT(7 == details.end_col);

    /* EOF ends input. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_EOF ==
            fido_scanner_read_token(&details, scanner));

    /* clean up. */
    fido_scanner_release(scanner);
}
