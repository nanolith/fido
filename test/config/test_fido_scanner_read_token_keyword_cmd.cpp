/**
 * \file test/config/test_fido_scanner_read_token_keyword_cmd.c
 *
 * \brief Test that we can read the "cmd" keyword.
 *
...
TEST_SUITE(fido_scanner_read_token_keyword_cmd);
...
TEST(basics)
{
    ...
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_KEYWORD_CMD ==
            fido_scanner_read_token_keyword_cmd(&details, scanner));
...
/**
 * \brief If this doesn't start with 'c', it's a failure.
 */
TEST(not_cmd)
{
    fido_scanner* scanner = nullptr;
