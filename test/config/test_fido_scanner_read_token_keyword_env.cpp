/**
 * \file test/config/test_fido_scanner_read_token_keyword_env.c
 *
 * \brief Test that we can read the "env" keyword.
 *
...
TEST_SUITE(fido_scanner_read_token_keyword_env);
...
TEST(basics)
{
    ...
    /* read the env keyword. */
    TEST_ASSERT(
        FIDO_SCANNER_TOKEN_TYPE_KEYWORD_ENV ==
            fido_scanner_read_token_keyword_env(&details, scanner));
...
/**
 * \brief If this doesn't start with 'e', it's a failure.
 */
TEST(not_env)
{
    fido_scanner* scanner = nullptr;
