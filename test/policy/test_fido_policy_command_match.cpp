...
/**
 * \brief Test that we can match a command with a wildcard.
 */
TEST(match_wildcard)
{
    fido_options* opts = nullptr;
...
/**
 * \brief Test that match fails if arguments are provided for a no args cmd.
 */
TEST(nomatch_no_arguments)
{
    fido_options* opts = nullptr;
    fido_config* config = nullptr;
    const char* TEST_INPUT = R"(
...
/**
 * \brief Test that we can fail to match a command with one exact argument.
 */
TEST(nomatch_one_exact_argument)
{
    fido_options* opts = nullptr;
    fido_config* config = nullptr;
    const char* TEST_INPUT = R"(
