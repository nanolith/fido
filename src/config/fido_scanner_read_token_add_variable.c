/**
 * \file config/fido_scanner_read_token_add_variable.c
 *
 * \brief Attempt to scan an add variable token.
 *
    ...
    fido_token_details* details, fido_scanner* scanner)
{
    ...
    retval =
        fido_scanner_complete_token_identifier(
            details, scanner, FIDO_SCANNER_TOKEN_TYPE_ADD_VARIABLE, true);
    goto done;
...
bad_input:
    ...
    MODEL_CONTRACT_CHECK_POSTCONDITIONS(
        fido_scanner_read_token_add_variable, retval, details, scanner);
