/**
 * \file config/fido_scanner_read_token_keyword_permit.c
 *
 * \brief Attempt to scan a permit keyword, falling back to username if 'r'
    ...
    fido_token_details* details, fido_scanner* scanner)
{
    ...
    /* for this function, the token must begin with 'p'. */
    if ('p' != *(scanner->input))
    {
        goto bad_input;
    }
    ...
    retval = fido_scanner_complete_token_keyword_permit(details, scanner);
    goto done;
...
bad_input:
    ...
    MODEL_CONTRACT_CHECK_POSTCONDITIONS(
        fido_scanner_read_token_keyword_permit, retval, details, scanner);
