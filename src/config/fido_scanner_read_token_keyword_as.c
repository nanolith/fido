/**
 * \file config/fido_scanner_read_token_keyword_as.c
 *
 * \brief Attempt to scan an as keyword, falling back to username if 'a'
    ...
    fido_token_details* details, fido_scanner* scanner)
{
    ...
    /* for this function, the token must begin with 'a'. */
    if ('a' != *(scanner->input))
    {
        goto bad_input;
    }
...
bad_input:
    ...
    MODEL_CONTRACT_CHECK_POSTCONDITIONS(
        fido_scanner_read_token_keyword_as, retval, details, scanner);
