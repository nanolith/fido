/**
 * \file config/fido_scanner_read_token_keyword_env.c
 *
 * \brief Attempt to scan an env keyword, falling back to username if 'r'
    ...
    fido_token_details* details, fido_scanner* scanner)
{
    ...
    /* for this function, the token must begin with 'e'. */
    if ('e' != *(scanner->input))
    {
        goto bad_input;
    }
...
bad_input:
    ...
    MODEL_CONTRACT_CHECK_POSTCONDITIONS(
        fido_scanner_read_token_keyword_env, retval, details, scanner);
